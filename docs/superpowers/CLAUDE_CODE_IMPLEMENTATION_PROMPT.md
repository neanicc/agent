# Codex / Claude Code Full Implementation Prompt

Copy everything inside the following block into Codex or Claude Code from the repository root.
...
- Detect whether the active implementation host is Codex or Claude Code, read the repository's
  `AGENTS.md`, and obey the closest applicable repository instructions.
- In Codex, use native planning, implementation, debugging, review, browser, and verification
  tools, including repository-installed gstack skills when they match the current task.
- In Claude Code, use the equivalent native tools and `superpowers:*` workflows when installed.
- A named skill, subagent workflow, MCP server, or plugin that is absent is not a blocker. Perform
  the equivalent bounded TDD, review, and verification steps directly and record the substitution.
- XcodeBuildMCP is optional. When it is unavailable, use reproducible `xcodebuild` and `simctl`
  commands and record the exact simulator/runtime selected.
...
3. Classify the working tree before acting. Do not stash, reset, clean, delete, overwrite, or rebase
   existing work.

   - Ignored OS/editor metadata such as `.DS_Store` is harmless: preserve it and continue.
   - Read and preserve root `AGENTS.md` whether tracked or generated by the user's gstack/Codex
     setup; never stage a user-owned ignored copy.
   - Preserve unrelated tracked or untracked user changes. If they do not overlap this program,
     record them and exclude them from every task commit. If they overlap or cannot be preserved
     safely, STOP and ask the user.
   - Merge conflicts, an in-progress Git operation, or ambiguous implementation changes require
     user direction before continuing.

4. Confirm the remote and main branch exist, then fetch without rewriting local work:
   git fetch origin

   - For a new implementation branch, require a clean non-ignored tree, switch to `main`, and run
     `git pull --ff-only origin main` before creating the branch.
   - For an existing `feat/loopguard-production`, resume it directly and compare its merge base to
     `origin/main`. Do not merge or rebase new main commits automatically; record the divergence
     and request direction only if it blocks the current task or creates a material conflict.
...
6. Never implement or push directly on main. Use the single integration branch
   ...
   - If it does not exist locally or remotely, create it only from the freshly updated local main:
...
Before implementing provider integrations, re-check these current official interfaces:
...
- Monitoring, routing features, change detection, test selection, and ordinary verification must
  not invoke a model.
- Attached integration distribution is plugin-first on both hosts. Direct settings-file merge is
  a mutually exclusive compatibility fallback with locking, validation, backup, atomic replace,
  checksum/version ownership, and exact uninstall/upgrade behavior. Never install both paths.
- Codex hook `PreToolUse` coverage is not a complete enforcement boundary. Managed Codex must
  initialize App Server with stable `clientInfo`, send `initialized`, discover models/capabilities,
  steer only an active turn with `expectedTurnId`, and use `thread/inject_items` between turns.
  Never silently substitute a semantically different method. Treat enterprise known-client access
  as an external launch gate.
- Managed Claude uses the documented Agent SDK streaming-input, effort/model discovery, interrupt,
  permission, and usage/cost surfaces. Keep installed-SDK compatibility fixtures authoritative.
...
Before Task 1, select and record a supported Python 3.11+ interpreter. Reuse a compatible existing
`loopguard/.venv` when its interpreter and lock/input fingerprint are valid. Otherwise create it
from the repository root with `python3 -m venv loopguard/.venv`; never delete or overwrite an
environment you do not own.
Resolve its absolute interpreter path once and use that path for every Python and pip command.
Never rely on bare `python`, `pip`, `pytest`, or `ruff` being on `PATH`.

Install only dependency groups that exist on main and run the current baseline, substituting the
resolved absolute virtualenv interpreter for `$PY` (for example,
`PY="$(pwd)/loopguard/.venv/bin/python"` on macOS/Linux, followed by `test -x "$PY"`):

cd loopguard
$PY -m pip install -e ".[dev,server,litellm]"
$PY -m pytest -q
...
by the plans. At the 2026-07-14 refresh, the expected baseline debt was two Ruff findings plus a
LoopGuard-owned unawaited broadcast warning; record fresh reality, and close any still-present
owned cases in `FOUNDATION-T06` as specified rather than misclassifying them as task regressions.
...
- Current plan and immutable task ID:
- Last completed step:
- Expected/observed last commit:
- Task dependencies and owned files:
- Red/green/regression commands with exit codes and counts:
- Evidence/artifact paths:
- Last successful cumulative verification:
- Known baseline failures:
- Active blockers:
- Next exact action:

Use immutable IDs with these exact prefixes: `FOUNDATION-T##`, `INTEGRATIONS-T##`, `VERIFY-T##`,
`CONTEXT-T##`, `ROUTING-T##`, `PREFERENCES-T##`, `BROWSER-T##`, `CLOUD-T##`, `SURFACE-T##`,
`AUTOHEAL-T##`, and `HARDENING-T##`. For example: `FOUNDATION-T01`, `INTEGRATIONS-T04`, and
`SURFACE-T12`. Task numbers do not change when a plan is executed in separate tranches. After
every task and before context compaction or session exit, update this file. On every resumed
...
Execute all 11 subsystem plans as these exact dependency-ordered tranches (88 numbered tasks):

1. `docs/superpowers/plans/2026-07-02-control-plane-foundation.md`
   Complete Tasks 1–6.

2. `docs/superpowers/plans/2026-07-02-agent-integrations.md`
   Complete attached/plugin Tasks 1–4 only.

3. `docs/superpowers/plans/2026-07-02-regression-verification.md`
   Complete Tasks 1–7.

4. `docs/superpowers/plans/2026-07-02-context-coordination.md`
   Complete Tasks 1–7.

5. `docs/superpowers/plans/2026-07-02-agent-integrations.md`
   Resume the same plan and complete managed/setup Tasks 5–8.

6. `docs/superpowers/plans/2026-07-02-model-routing-cost-control.md`
   Complete Tasks 1–7.

7. `docs/superpowers/plans/2026-07-02-preference-engine.md`
   Complete Tasks 1–6.

8. `docs/superpowers/plans/2026-07-02-browser-acceleration.md`
   Complete Tasks 1–7.

9. `docs/superpowers/plans/2026-07-02-cloud-control-plane.md`
   Complete Tasks 1–9.

10. `docs/superpowers/plans/2026-07-02-control-surfaces.md`
    Complete core client Tasks 1–11 only. Package and verify both clients with repair navigation
    absent while the effective repair capability is disabled.

11. `docs/superpowers/plans/2026-07-02-auto-heal-pipelines.md`
    Complete Tasks 1–9, activating the workflow-backed repair capability only when ready.

12. `docs/superpowers/plans/2026-07-02-control-surfaces.md`
    Resume the same plan and complete repair-activation Task 12, then rerun final client parity,
    visual, accessibility, and package gates.

13. `docs/superpowers/plans/2026-07-02-production-hardening.md`
    Complete Tasks 1–10.

Do not reorder these tranches and do not implement later work early merely because its files look
convenient. If an undeclared later API is required, treat that as a dependency defect: explain it,
fix the owning plan/order and contract tests first, then continue. The cloud plan owns the disabled
repair schema; Auto-Heal owns its implementation/readiness; Surface Task 12 owns client activation.
...
- One milestone tag in the progress file per completed tranche, including cumulative test evidence.
- Update the draft PR body after each tranche with a collapsible task/commit/test section.
- Before starting the next tranche, verify every earlier tranche's cumulative gate, schema/API
  contract compatibility, migration head, and clean working tree.
...
Use the active host's installed executing-plan workflow when available. For implementation and bug
fixes, use test-driven development. Use systematic root-cause debugging for unexpected failures
and fresh verification before every completion claim. Missing named workflows are never blockers.
...
Subagents may implement or review one bounded task at a time only when the active host and repository
instructions authorize delegation. Never allow two agents to edit the same files simultaneously.
The primary agent must review every delegated diff and rerun its verification before accepting it.

PLAN COMPLETION RULES

After every execution tranche:

1. Run that tranche's owning plan gate, narrowed only when the plan explicitly defines a partial
   gate; run the full gate when the last tranche of that plan completes.
...
4. Update the progress file with the tranche result and next exact tranche.
...
7. If `gh` is authenticated:
   - Create one draft PR after the first completed tranche if none exists.
   - Otherwise update the existing draft PR body.
   - Do not create unrelated PRs and do not merge the draft PR.
8. Continue immediately into the next tranche unless a real blocker requires user authority.
...
- Product go/no evidence and whether the next external rollout remains `NO-GO`

PRODUCT AND RELEASE EVIDENCE

- Preserve the user-selected one-branch/one-draft-PR workflow, but keep every task independently
  reviewable through its immutable task ID, scoped commit, evidence paths, and cumulative gate.
- At the milestones named in the master plan, record activation, retained weekly use, proof-result
  usefulness, inconclusive rate, false-block rate, prevented-waste ratio, proof engagement, and
  overhead against the exact go/no thresholds. Never replace observed measurements with estimates.
- Missing cohort evidence does not block implementation, but it keeps public beta or GA `NO-GO`.
  Record the owner, cohort, collection window, and next evidence action instead of self-waiving it.
- Before public naming, distribution, billing, or launch assets, require the master plan's explicit
  product name/positioning, trademark/legal, registry ownership, support, privacy, and terms gates.
...
- Required macOS/Xcode tooling for iOS verification is unavailable after trying documented
  `xcodebuild`/`simctl` fallbacks. Do not mark iOS complete or silently skip required gates.
...
After all 11 subsystem plans and all 88 numbered tasks:
...
- Every completed plan, tranche, immutable task ID, and task count (11 plans / 88 tasks)
...
- The full 13-tranche / 11-plan / 88-task execution order
