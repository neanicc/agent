...
_RULE_CAPABILITIES = {
    "wcag-contrast": ["verification", "repair publication"],
    "loop-threshold": ["loop intervention"],
}
...
class ControlQueryService:
    ...
    def read_preferences(self, tenant_id: uuid.UUID) -> dict[str, Any]:
        return self._profiles.get(
            tenant_id,
            {
                ...
                "rules": [
                    self._policy_rule(rule_id, severity, managed=True)
    ...
    def write_preferences(
        ...
        supplied = {str(rule.get("id")): str(rule.get("severity")) for rule in rules}
        for rule_id, minimum in self.managed_rules.items():
            severity = supplied.get(rule_id)
            if (
                severity is None
                or severity not in _SEVERITY
                or _SEVERITY[severity] < _SEVERITY[minimum]
            ):
        ...
        profile = {
            ...
            "rules": [
                self._policy_rule(
                    str(rule["id"]),
                    str(rule["severity"]),
                    managed=str(rule["id"]) in self.managed_rules,
                )
                for rule in rules
            ],
    ...
    def _policy_rule(self, rule_id: str, severity: str, *, managed: bool) -> dict[str, Any]:
        return {
            "id": rule_id,
            "severity": severity,
            "managed": managed,
            "source": "organization" if managed else "profile",
            "precedence": "managed minimum" if managed else "profile override",
            "affected_capabilities": _RULE_CAPABILITIES.get(rule_id, ["evaluation"]),
        }
    ...
    def hosts(self, tenant_id: uuid.UUID) -> list[dict[str, Any]]:
        return [
            self._host_dict(host) for host in self._hosts.values() if host.tenant_id == tenant_id
        ]
    ...
    def capabilities(self, principal: Principal, host_id: uuid.UUID) -> dict[str, Any]:
        ...
        if stale:
            ...
            return {
                ...
                "status": "degraded",
                "features": unavailable,
                "observed_at": host.observed_at.isoformat(),
                "ttl_seconds": host.ttl_seconds,
            }
        can_control = Permission.CONTROL_SESSION in principal.permissions
        bound = host.repository_bound
        features = {
            "remote_actions": {
                "available": can_control and bound,
    ...
    def add_resource(self, kind: str, *, tenant_id: uuid.UUID, values: dict[str, Any]) -> uuid.UUID:
        ...
        self._resources[kind].append(
            {"id": resource_id, "tenant_id": tenant_id, "created_at": self._clock(), **values}
        )
        return resource_id

    def resources(
        self, kind: str, tenant_id: uuid.UUID, *, limit: int = 50
    ) -> list[dict[str, Any]]:
    ...
    def resource(
        self, kind: str, tenant_id: uuid.UUID, resource_id: uuid.UUID
    ) -> dict[str, Any] | None:
        return next(
            (
                self._public(item)
                for item in self._resources[kind]
                if item["tenant_id"] == tenant_id and item["id"] == resource_id
            ),
            None,
        )
    ...
    def encode_cursor(created_at: datetime, resource_id: uuid.UUID) -> str:
        return base64.urlsafe_b64encode(
            json.dumps([created_at.isoformat(), str(resource_id)], separators=(",", ":")).encode()
        ).decode()
    ...
    def _host_dict(self, host: HostView) -> dict[str, Any]:
        return {
            ...
            "repository_bound": host.repository_bound,
        }
